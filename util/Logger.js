...
function PrefixLogger(logger, prefix, debug = false) {
  // The callable form: logger(level, msg) — used by inner PrefixLogger instances.
  const call = (level, msg, ...args) => {
    if (typeof logger === "function") {
      logger(level, `[${prefix}] ${msg}`, ...args);
    } else if (logger && typeof logger.info === "function") {
      // Inner logger is itself a PrefixLogger (function-object); call via named methods.
      if (level === "debug") { logger.debug(`[${prefix}] ${msg}`, ...args); }
      else if (level === "warn") { logger.warn(`[${prefix}] ${msg}`, ...args); }
      else if (level === "error") { logger.error(`[${prefix}] ${msg}`, ...args); }
      else { logger.info(`[${prefix}] ${msg}`, ...args); }
    }
  };
  ...
  call.debug = (...args) => { if (debug) call("debug", args[0], ...args.slice(1)); };
